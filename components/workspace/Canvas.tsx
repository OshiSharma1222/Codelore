...
export function Canvas({ children }: CanvasProps) {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      if (e.ctrlKey) {
        e.preventDefault();
        setZoom(prev => Math.min(Math.max(prev - e.deltaY * 0.001, 0.4), 2));
      }
    };
    ...
    >
      ...
      <div className="absolute bottom-6 left-6 flex items-center gap-2 z-10">
        <div className="bg-white border-2 border-black flex gap-1 p-1 shadow-[4px_4px_0px_black]">
          <ControlButton icon={<RotateCcw size={14} />} onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }} />
          <div className="w-[1px] bg-zinc-200 mx-1" />
          <ControlButton icon={<Minimize2 size={14} />} onClick={() => setZoom(z => Math.max(z - 0.1, 0.4))} />
          <span className="text-[10px] font-bold w-10 text-center flex items-center justify-center">
            {Math.round(zoom * 100)}%
          </span>
          <ControlButton icon={<Maximize2 size={14} />} onClick={() => setZoom(z => Math.min(z + 0.1, 2))} />
        </div>
